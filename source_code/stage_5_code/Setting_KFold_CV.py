...
from source_code.stage_5_code.GCN_Model import run
...
class Setting_KFold_CV(setting):
    ...
    def load_run_save_evaluate(self):
        ...
        if self.data_split:
            ...
            learned_result = run(data=data, model=self.method)
            ...
            # learned_result = run(data=data, model=self.method, dataset=self.dataset)
            # return learned_result
            data = self.dataset
            learned_result = run(data_dict=data, model=self.method)
            # save raw ResultModule
            self.result.data = learned_result
            # self.result.fold_count = fold_count
            self.result.save()
